/* ==========================================================================
   ...
   Depends on: firebase/firebase-config.js (auth, googleProvider, db)

   [Prompt 8 update] Added a small Firestore hook (`ensureUserProfile`) that
   creates/updates a `users/{uid}` profile document on every sign-in. This is
   the one necessary change to a previous-prompt file in this module — the
   Admin Panel's "View All Users" / suspend / role management needs real
   user documents to operate on, and nothing before this wrote any. New
   users default to role: 'member', status: 'active'; an admin later flips
   role to 'admin' by hand (or via a one-time Firestore console edit) since
   there's no self-serve "become an admin" button. Everything else in this
   file — every form, validator, and redirect — is unchanged from Prompt 2.
   ========================================================================== */
...
(() => {
  ...
  /* ================================================== USER PROFILE DOC === */
  // Firestore users/{uid} — the record the Admin Panel's user management
  // reads/writes. Creates the doc with sane defaults on first sign-in;
  // on every later sign-in it only refreshes name/email/lastLogin so an
  // admin-assigned role or suspended status is never silently overwritten.
  async function ensureUserProfile(user) {
    ...
    try {
      ...
      if (!snap.exists()) {
        await setDoc(ref, {
          uid: user.uid,
          name: user.displayName || 'Iron Edge Member',
          email: user.email || '',
          role: 'member',
          status: 'active',
        ...
        await setDoc(
          ref,
          {
            name: user.displayName || snap.data().name || 'Iron Edge Member',
      ...
      // Non-fatal — the person can still use the site even if this write fails.
      console.warn('[IronEdge auth] could not sync user profile:', err);
    }
  }
